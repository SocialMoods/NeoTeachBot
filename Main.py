...
def extract_text_from_image_with_gpt(image_path):
    ...
    response = openai.ChatCompletion.create(
        ...
        messages=[
            {
                "role": "system",
                "content": "You are a cool image analyst. Your goal is to read the text from the image provided."
            },
            {
                "role": "user",
                "content": "Here is the image. Can you recognize the text?",
            },
            {
                "role": "user",
                "content": {
...
def start_command(message):
    if is_admin(message.from_user.id):
        bot.send_message(message.chat.id,
                         "<b>Привет!</b>\n\nДля того, чтобы добавить новый конспект, напиши название предмета!",
                         parse_mode="HTML")
    else:
        bot.send_message(message.chat.id,
                         "<b>Привет!</b>\n\nЧтобы посмотреть конспекты на конкретную дату, напиши /view!\n\n<i>Конспекты иногда могут не загружаться или отсутствовать. Просим заранее прощения</i>",
                         parse_mode="HTML")


@bot.message_handler(
    func=lambda message: is_admin(message.from_user.id) and message.from_user.id not in admin_state and message.text)
def handle_subject_input(message):
    ...
    if result[0] > 0:
        bot.send_message(message.chat.id,
                         f"<b>Ошибка!</b>\n\nКонспект для предмета {subject} на сегодняшнюю дату ({date}) уже существует.",
                         parse_mode="HTML")
        return

    bot.send_message(message.chat.id,
                     f"<b>Вы ввели предмет: {subject}</b>\n\nСколько фотографий вы хотите загрузить? Введите числом",
                     parse_mode="HTML")
...
@bot.message_handler(func=lambda message: message.from_user.id in admin_state and admin_state[message.from_user.id][
    'photo_count'] == 0 and message.text.isdigit())
def handle_photo_count_input(message):
    count = int(message.text)
    if count > 0:
        admin_state[message.from_user.id]['photo_count'] = count
        bot.send_message(message.chat.id,
                         f"<b>Вы выбрали {count} фотографий.</b>\n\nТеперь отправьте фотографии одну за другой.",
                         parse_mode="HTML")
    else:
...
@bot.message_handler(func=lambda message: message.from_user.id in admin_state and admin_state[message.from_user.id].get(
    'awaiting_handwriting_check'))
def handle_handwriting_check(message):
    ...
    if answer == "да":
        ...
        # Используем GPT-4 для извлечения текста и создания изображения с печатным текстом
        photo_path = subject_data['last_photo_path']

        # Используем GPT-4 для распознавания текста
        recognized_text = extract_text_from_image_with_gpt(photo_path)
...
def finish_adding_notes(message):
    ...
    bot.send_message(message.chat.id, f"<b>Успех!</b>\n\nКонспект для предмета {subject} на {date} успешно добавлен.",
                     parse_mode="HTML")
...
def view_notes(message):
    bot.send_message(message.chat.id,
                     "<b>Просмотр конспектов</b>\n\nВведите дату в формате ДД.ММ.ГГГГ, чтобы увидеть доступные конспекты",
                     parse_mode="HTML")
...
def handle_subject_selection(call):
    ...
    if photos:
        ...
        bot.send_message(call.message.chat.id, f"Конспекты для предмета {subject} за {input_date} не найдены.",
                         parse_mode="HTML")
